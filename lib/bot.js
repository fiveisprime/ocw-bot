...
Bot.prototype.handleMessage = function(from, to, text) {
  if (/\b(ocw|icw|java|svn)\b/gi.test(text)) this.say(this.lols[~~(Math.random() * this.lols.length)]);
  ...
  if (/\b(ux)\b/gi.test(text)) this.say('just do it in powerpoint ' + this.lols[~~(Math.random() * this.lols.length)]);

  if (/\b(c scott|c. scott)\b/gi.test(text)) this.say('he\'s even cooler than Peter!');

  if (/\b(scott)\b/gi.test(text)) this.say( this.scott[~~(Math.random() * this.scott.length)]);
