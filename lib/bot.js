...
//
Bot.prototype.sayText = function(searchString) {
    if (typeof searchString === 'string' ) {
      var filteredtexts=texts.filter( function(item) {
          return item.contains(searchString);
        });
      if (filteredtexts.length>0)
	{
        this.say(filteredtexts[~~(Math.random() * filteredtexts.length)]);
      }
	else {
