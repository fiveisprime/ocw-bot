...
Bot.prototype.handleJoin = function(channel, nick) {
  ...
  automeme.getMeme()
    .then(function(meme) {
      _this.say('hey ' + nick + ', ' + meme.toLowerCase().replace('\n', ''));
    })
    .fail(console.error);
};
...
Bot.prototype.handleMessage = function(from, to, text) {
  ...
  if (/webbuilder|WebBuilder/gi.test(text)) this.say('oh! you mean ' + this.webbuilder[~~(Math.random() * this.webbuilder.length)] + ', right?');

  if (/lolbot/gi.test(text)) automeme.getMeme().then(this.say.bind(this)).fail(console.error);
};
