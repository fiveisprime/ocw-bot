...
function Bot(client) {
  ...
  this.namePattern = /\b(lolbot)\b/gi;
  this.lols = [
    'lol'
  , 'lolol'
  , 'lol!'
  ...
  this.client.on('registered', function() {
    this.name = arguments[0].args.length > 0 && arguments[0].args[0];
    this.namePattern = new RegExp('\\b(' + this.name + ')\\b', 'gi');
  }.bind(this));
}
...
Bot.prototype.handleJoin = function(channel, nick) {
  ...
  //
  if (this.namePattern.test(nick)) return;
...
Bot.prototype.handleMessage = function(from, to, text) {
  if (/\b(ocw|icw|intelligrated|java|svn)\b/gi.test(text)) this.say(this.lols[~~(Math.random() * this.lols.length)]);

  if (/\b(ross)\b/gi.test(text)) this.say('omg... lol');

  if (/\b(jamie)\b/gi.test(text)) this.say('your opinion has been noted ' + this.lols[~~(Math.random() * this.lols.length)]);

  if (/\b(ken)\b/gi.test(text)) this.say('he\'s got those moves like ken mcjagger.. lol');

  if (/\b(scott|ux)\b/gi.test(text)) this.say('just do it in powerpoint ' + this.lols[~~(Math.random() * this.lols.length)]);

  if (/\b(beer)\b/gi.test(text)) this.say('I\'ll take one too');

  if (/\b(emacs)\b/gi.test(text)) this.say('lol   code         formatting. ..');

  if (/\b(peter)\b/gi.test(text)) this.say('he\'s the coolest');

  if (/\b(meerkat|meekrat)\b/gi.test(text)) this.say('JamieScript ' + this.lols[~~(Math.random() * this.lols.length)]);

  if (/\b(webbuilder)\b/gi.test(text)) this.say('oh! you mean ' + this.webbuilder[~~(Math.random() * this.webbuilder.length)] + ', right?');

  if (this.namePattern.test(text)) ~~(Math.random() * 2) ? this.sayText() : this.sayMeme();

  if (/\b(mike)\b/gi.test(text)) this.say('I am NOT slow.');

  if (/\b(doug)\b/gi.test(text)) this.say('The Doug has spoken.');

  if (/\b(taco)\b/gi.test(text)) this.say('Nobody told me it was a requirement.');

  if (/\b(!text)\b/gi.test(text)) this.sayText();

  if (/\b(wat)\b/gi.test(text)) this.sayWat();
};
